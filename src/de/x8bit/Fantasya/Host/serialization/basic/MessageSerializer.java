...
public class MessageSerializer implements ObjectSerializer<Message> {
	...
	public Message load(Map<String, String> mapping) {
		...
		Message msg = null;
		boolean TimeStampParseProblem = false;
		try {
	    ...
	    try {
      Timestamp mysqlDate = Timestamp.valueOf(mapping.get("zeit"));
      msg.setTimestamp(new Date(mysqlDate.getTime()));
  } catch (IllegalArgumentException ex) {
      if (!TimeStampParseProblem) {
			new SysErr("PROBLEM mit Timestamp '" + mapping.get("zeit") + "' bei den Meldungen - wenn das außerhalb des Mischbetriebs ZAT/EVA passiert, dann ist es ein Grund nachzuschauen! - " + ex);
			TimeStampParseProblem = true;
		}
      msg.setTimestamp(new Date(0));
  }
	...
	public SerializedData save(Message object) {
		...
		data.put("debuglevel", "0");
		data.put("zeit", new Timestamp(object.getTimeStamp().getTime()).toString());
